...
2. **Destination**: which topic and which file in `notes/` (e.g. `topics/fundamentals_software_architecture/notes/02-fundamentos-razonamiento-arquitectonico.md`). If the user is already working on a note in the conversation, use that one without asking again.
