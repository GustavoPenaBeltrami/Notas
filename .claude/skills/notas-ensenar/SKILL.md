---
name: notas-ensenar
description: Enseña un tema para que quede entendido y no memorizado — sondea el nivel real con preguntas corregidas, arma un plan como grafo de dependencias, y construye nodo por nodo desde verdades incondicionales. Usar SIEMPRE que haya que explicar algo, desde una aclaración de una línea hasta una sesión larga. Se dispara con "/notas-ensenar", "enseñame", "explicame", "no entiendo", "tomame el tema", o cuando el usuario pega material de estudio y pide entenderlo.
---
...
graph TD
  A[Paquete] --> B[Orden]
  A --> C[Reenvío ante pérdida]
  B --> D[Stream confiable]
  C --> D
```
````
