...
def test_fonts():
    ...
    real_root, store.ROOT = store.ROOT, tmp
    try:
        assert store.list_fonts() == []
        assert store.save_font("My  Font.woff2", WOFF2) == {"fonts": ["my-font.woff2"]}, "lowercase, spaces to -"
        store.save_font("b.ttf", b"\0\1\0\0" + b"\0" * 60)
        store.save_font("c.otf", b"OTTO" + b"\0" * 60)
        assert (tmp / "fonts" / "my-font.woff2").read_bytes() == WOFF2
        assert store.list_fonts() == ["b.ttf", "c.otf", "my-font.woff2"]
        assert 'url("/fonts/b.ttf")' in store.fonts_css() and ':root[data-font="my-font.woff2"]' in store.fonts_css()
        ...
        assert rejected("big.woff2", WOFF2 + b"\0" * store.BODY_LIMIT), "size cap"
        for name in ("../evil.ttf", "../../fonts/x.woff2", "a/b.otf", "..\\x.ttf", ".hidden.ttf", "/etc/x.ttf"):
            assert rejected(name, WOFF2), "path traversal: " + name
        assert store.list_fonts() == ["b.ttf", "c.otf", "my-font.woff2"], "a rejected upload must not write"
