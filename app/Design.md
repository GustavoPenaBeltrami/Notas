...
- Names written into the note HTML stay as they were so saved `.md` files keep rendering: `mk`, `mk-fondo`, `mk-sub`, `mk-tacha`, `data-nota`, `tarjeta`, `cuerpo`, `inline`, `ref`, `viz`, `data-kind`, `src`, `view`, `data-x`/`data-y` and the inline `--c`. Renaming any of them needs a migration of every note.
