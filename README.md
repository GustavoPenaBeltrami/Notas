...
Sistema de estudio para el roadmap de arquitectura de software.
...
```
README.md            Esto.
package.json         Los dos comandos. Sin dependencias.
.claude/
    skills/          Las skills de abajo. Se activan solas en este directorio.
    agents/          investigador (verifica antes de enseñar) y un profesor-<tema> por cada tema con agente dedicado.
app/
...
por área: `notas-init` ofrece generar `.claude/agents/profesor-<tema>.md` al
...
El agrupamiento vive en `temas/fundamentals_software_architecture/progreso/roadmap.md`.

## Skills de Claude Code

Viven en `.claude/skills/`. Se activan solas cuando el pedido encaja, o las
llamás por nombre con `/`. Todas escriben en las carpetas de este proyecto, no
en el chat.
...
Más el subagente **investigador** (`.claude/agents/`), que no se llama a mano:
...
Editá `.claude/skills/notas-ensenar/SKILL.md` si querés que enseñe distinto:
